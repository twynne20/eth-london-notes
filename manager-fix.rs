...
impl SubnetManager for EthSubnetManager {
    ...
    async fn join_subnet(
        ...
        // Convert the Filecoin address to an Ethereum address
        let evm_address = payload_to_evm_address(from.payload())?;
    
        // Get the current nonce for the sender's address
        let current_nonce = signer.get_transaction_count(evm_address, None).await?;
        // Increment the nonce by 1
        let next_nonce = current_nonce + 1;
        // Set the incremented nonce in the transaction options
        txn.tx.set_nonce(next_nonce);
...
async fn premium_estimation(
    ...
    let max_fee_per_gas = ethers::types::U256::from(5_000_000u64); // 100 GWei
    ...
    let max_priority_fee_per_gas = ethers::types::U256::from(5_000_000u64);
